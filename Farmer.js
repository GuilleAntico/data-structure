...
class Farmer {
    ...
    filterByCropType( cropType, year ){
        try {
            ...
            this.farmers.forEach(farmer => {
                ...
                if(farmer.farms.length){
                    farmer.farms.forEach( farm => {
                        ...
                        const result = farm.plantingEvents.filter( pe => pe.cropType === cropType && this._checkIfSameYear(year, pe.date));
                        // now i want to sort Desc the planting events so i can be sure the first item is the most recent
                        if(result && result.length) plantingEvents = this._sortPlantingEventDesc(result)
                    });
                }
            ...
            return this._sortNamesByPlantingDateDesc(names);
        }catch(error){
            throw error;
        }
    }
    // naming convention for internal class method
    _sortPlantingEventDesc(plantingEvents){
    ...
    // naming convention for internal class method
    _sortNamesByPlantingDateDesc(farmers){
